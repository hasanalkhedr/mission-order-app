<div id="deleteRateModal-{{ $chancelleryRate->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="flex justify-end items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                <div>
                    <button data-modal-toggle="deleteRateModal-{{ $chancelleryRate->id }}" type="button"
                        class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">
                        {{ __('Cancel') }}
                    </button>
                </div>
                <div>
                    <form method="POST" action="{{ route('chancelleryRates.destroy', ['chancelleryRate' => $chancelleryRate->id])}}">
                        @csrf
                        @method('DELETE')
                        <button data-modal-toggle="deleteRateModal-{{ $chancelleryRate->id }}"
