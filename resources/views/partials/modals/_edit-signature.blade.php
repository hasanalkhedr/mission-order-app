<div id="editSignatureModal-{{ $signature->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="flex justify-between items-center p-4 rounded-t border-b">
                <div class="text-base font-bold mt-3 sm:mt-0 sm:ml-4 sm:text-left blue-color">
                    {{ __('Edit Signature') }}: {{ $signature->employee->first_name }}
            ...
            <div class="p-6 overflow-y-auto" style="max-height: 500px">
                <form method="POST" action="{{ route('signatures.update', ['signature' => $signature->id]) }}"
                    id="edit_form--{{ $signature->id }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="w-1/2 px-3">
                            ...
                            <select name="employee_id" id="employee_id"
                                ...
                                @forelse($employees as $employee)
                                    <option value="{{ $employee->id }}" @selected(old('employee_id', $signature->employee_id ?? '') == $employee->id)>
                        ...
                        <div class="w-1/2 px-3 h-1/3">
                            ...
                            <div class="relative w-full h-full mx-auto">
                                ...
                                <div
                                    ...
                                    <input type="file" name="signature_path"
                    ...
                    <div class="flex justify-end items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                        <div>
                            <button
                                ...
                                data-modal-toggle="editSignatureModal-{{ $signature->id }}">{{ __('Edit') }}
                            </button>
                        </div>
                        <div>
                            <button data-modal-toggle="editSignatureModal-{{ $signature->id }}" type="button"
