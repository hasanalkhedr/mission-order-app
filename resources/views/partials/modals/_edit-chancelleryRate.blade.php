<div id="editRateModal-{{ $chancelleryRate->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="p-6 overflow-y-auto" style="max-height: 500px">
                <form method="POST" action="{{ route('chancelleryRates.update', ['chancelleryRate' => $chancelleryRate->id]) }}"
                    id="edit_form--{{ $chancelleryRate->id }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {{-- <div class="form-group">
                                ...
                                <input type="number" step="0.0001" class="form-control" id="usd_rate" name="usd_rate"
                                    value="{{ $chancelleryRate->usd_rate }}" required>
                                <small class="form-text text-muted">
                                    {{ __('1 USD = ? INR') }}
                                </small>
                            </div> --}}
                    ...
                    <div class="flex justify-end items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                        <div>
                            <button
                                class="text-white bg-bkue-700 hover:bg-blue-480 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center"
                                data-modal-toggle="editRateModal-{{ $chancelleryRate->id }}">{{ __('Update') }}
                            </button>
                        </div>
                        <div>
                            <button data-modal-toggle="editRateModal-{{ $chancelleryRate->id }}" type="button"
