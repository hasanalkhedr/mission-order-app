<div id="deleteSignatureModal-{{ $signature->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="flex justify-between items-center p-4 rounded-t border-b">
                ...
                <div class="text-base font-bold mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                    {{ __('Delete Signature') }}: {{ $signature->employee->first_name }} {{ $signature->employee->last_name }}
                </div>
                <div>
                    <button type="button"
            ...
            <div class="p-6 space-y-6">
                <div class="text-base leading-relaxed text-gray-500">
                    {{ __('Are you sure you want to delete this signature') }}?
            ...
            <div class="flex justify-end items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                <div>
                    <button data-modal-toggle="deleteSignatureModal-{{ $signature->id }}" type="button"
                        class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">
                        {{ __('Cancel') }}
                    </button>
                </div>
                <div>
                    <form method="POST" action="{{ route('signatures.destroy', ['signature' => $signature->id])}}">
                        @csrf
                        @method('DELETE')
                        <button data-modal-toggle="deleteSignatureModal-{{ $signature->id }}"
