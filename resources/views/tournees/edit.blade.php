...
@section('content')
    ...
    <form id="mainForm" action="{{ route('tournees.update', $tournee->id) }}" method="POST" class="w-full">
        @csrf
        @method('PUT')
        ...
        {{-- <div class="flex flex-wrap -mx-3 mb-2">
            <div class="w-full px-3">
                ...
                <textarea name="purpose" rows="2" required minlength="100"
                    ...
                    oninput="updateCharCounter(this)">{{ old('purpose', $tournee->purpose) }}</textarea>
            ...
            <div class="w-full px-3 py-1">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="if_conge" id="if_conge"
                        value="1" @checked(old('if_conge', $tournee->conge ? 1 : 0) > 0)
        ...
        <div class="flex flex-wrap -mx-3 mb-2" id="conge_container" style="display: none;">
            <div class="w-1/2 px-3">
                <textarea class="appearance-none block w-full bg-white text-gray-700 rounded py-3 px-4 mb-1 leading-tight focus:outline-none border border-blue-700 focus:bg-white focus:border-blue-900"
                        name="conge" id="conge_input" disabled>{{old('conge',$tournee->conge)}}</textarea>
            </div>
        </div>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
        ...
        <div x-data="destinationManager({{ $tournee->tourneeDestinations->toJson() }}, {{$tournee->has_weekend}})">
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('destinationManager', (initialDestinations = null, hasWeekend = false) => ({
...
*/
        ...
        <div class="flex flex-wrap -mx-3 mb-2">
            ...
            <div class="w-full px-3 py-1">
                ...
                <input required @checked(old('charge1', $tournee->charge1) == 1) type="radio" value="1" name="charge1"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border border-blue-700 focus:ring-blue-500 dark:focus:ring-blue-600 mr-0 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label class="ms-1 text-sm font-medium text-blue-500 dark:text-gray-500 mr-5">OUI</label>
                <input required @checked(old('charge1', $tournee->charge1) == 0) type="radio" value="0" name="charge1"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border border-blue-700 focus:ring-blue-500 dark:focus:ring-blue-600 mr-0 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label class="ms-1 text-sm font-medium text-blue-400 dark:text-gray-500 mr-10">NON</label>
            </div>
            <div class="w-full px-3 py-1">
            ...
            <div class="w-full px-3 py-1">
                ...
                <input required @checked(old('repas', $tournee->repas) == 1) type="radio" value="1" name="repas"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border border-blue-700 focus:ring-blue-500 dark:focus:ring-blue-600 mr-0 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label class="ms-1 text-sm font-medium mr-5 text-blue-400 dark:text-gray-500">OUI</label>
                <input required @checked(old('repas', $tournee->repas) == 0) type="radio" value="0" name="repas"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border border-blue-700 focus:ring-blue-500 dark:focus:ring-blue-600 mr-0 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                <label class="ms-1 text-sm font-medium text-blue-400 dark:text-gray-500 mr-5">NON</label>
            </div>
        </div>
                ...
                Alpine.data('expensesManager', (initialExpenses = null) => ({
                    expenses: initialExpenses ?
                                initialExpenses.map(dest => ({
                                    ...dest,
                                    passenger: dest.passenger == '1' || dest.passenger === true,
                                    distance: dest.distance == '1' || dest.distance === true,
                                    material: dest.material == '1' || dest.material === true,
                                    visits: dest.visits == '1' || dest.visits === true
                                })) :
                     [{
                        type: '',
                        transport_type: '',
                    ...
                    init() {
                        ...
                        @if(old('expenses'))
                            this.expenses = @json(old('expenses')).map(expense => ({
                                ...expense,
                                passenger: expense.passenger == '1' || expense.passenger === true,
                                distance: expense.distance == '1' || expense.distance === true,
                                material: expense.material == '1' || expense.material === true,
                ...
                <div class="flex justify-center items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                    ...
                    <div>
                        <button class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-bold rounded-lg text-md w-48 px-5 py-2.5 text-center"
                            type="button"  data-modal-toggle="deleteModal-{{ $tournee->id }}">{{ __('Delete') }}
                        </button>
