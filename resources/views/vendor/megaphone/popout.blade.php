<div x-cloak x-show="open" class="fixed w-full h-full top-0 left-0 z-40 bg-black bg-opacity-20" @click="open = false"
    x-transition.opacity.duration.600ms></div>

<div x-cloak x-show="open" x-transition:enter="transform transition ease-in-out duration-300 sm:duration-300"
    x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
    x-transition:leave="transform transition ease-in-out duration-300 sm:duration-300"
    x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" @click.outside="open = false"
    ...
    <div class="w-full bg-white shadow h-screen overflow-y-auto absolute right-0 z-30">
        <div class="flex items-center justify-between border-b border-gray-200 p-4">
            ...
            <button @click="open = false"
                class="absolute top-0 right-0 z-30 flex items-center justify-center px-2 py-1 mt-4 mr-5 space-x-1 text-xs font-medium uppercase border rounded-md border-neutral-200 text-neutral-600 hover:bg-neutral-100">
                ...
                <div class="border-b pb-2 border-gray-300 text-gray-600 flex justify-between">
                    ...
                    @if ($unread->count() > 1)
                        <button class="focus:outline-none text-sm leading-normal pt-8 hover:text-indigo-700"
                            wire:click="markAllRead()">Mark all as read</button>
                    @endif
                </div>

                @foreach ($unread as $announcement)
                    <div
                        class="w-full p-3 mt-4 bg-white rounded-xl flex flex-shrink-0 {{ $announcement->read_at === null ? 'drop-shadow shadow border' : '' }}">
                        ...
                        @if ($announcement->read_at === null)
                            <button role="button" aria-label="Mark as Read"
                                class="absolute top-0 right-0 outline-none px-1 py-1 mt-2 mr-2 space-x-1 cursor-pointer border rounded-md border-neutral-200 text-neutral-600 hover:bg-neutral-100"
